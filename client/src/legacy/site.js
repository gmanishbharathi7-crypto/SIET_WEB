...
function internalPage(route) { const dept = route.startsWith('department/') ? titleCase(route.slice(11).replaceAll('-', ' ')).replaceAll(' And ', ' & ') : ''; if (dept) return departmentPage(dept); const data = pageCopy[route] || ['Sri Shakthi', 'Institutional information', 'Explore Sri Shakthi Institute of Engineering and Technology.']; return `<main class="internal-page"><section class="page-hero"><img class="page-crest" src="/brand/siet-logo.png" alt=""><div class="eyebrow"><span></span> SRI SHAKTHI</div><h1 class="reveal">${data[0]}</h1><p>${data[1]}</p></section><section class="page-content"><div class="reveal"><div class="section-no">OVERVIEW</div><h2>${data[1]}</h2><p>${data[2]}</p>${route === 'contact' ? '<div class="contact-panel"><b>Sri Shakthi Institute of Engineering & Technology</b><p>Sri Shakthi Nagar, L&T By-Pass, Chinniyampalayam, Coimbatore – 641062</p><p>+91 422 2369900 · info@siet.ac.in</p></div>' : ''}<a class="button" href="#/admission-enquiry">Enquire now ${icon('arrow')}</a></div></section>${['departments', 'programmes'].includes(route) ? `<section class="page-content programme-content"><div class="section-no">PROGRAMMES &amp; DEPARTMENTS</div><div>${programs.map(([n, d, img]) => `<a class="flip-card" href="#/department/${slugify(n)}"><span class="flip-card-inner"><span class="flip-front"><small>DEPARTMENT</small><b>${n}</b><p>${d}</p><span>Explore department →</span></span><span class="flip-back" style="background-image:linear-gradient(180deg,transparent,rgba(3,45,27,.94)),url('${img}')"><b>${n}</b></span></span></a>`).join('')}</div></section>` : ''}</main>` }
const titleCase = s => s.replace(/\b\w/g, c => c.toUpperCase());

const acadIcon = (name) => ({
  courses: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>`,
  faculty: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m22 10-10-5L2 10l10 5 10-5Z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/><path d="M22 10v6"/></svg>`,
  labs: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 2v7.31L4.15 19.5A2 2 0 0 0 5.86 22h12.28a2 2 0 0 0 1.71-2.5L14 9.31V2"/><path d="M8.5 2h7"/><path d="M14 9.3 8.5 18"/></svg>`,
  industry: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="2" width="16" height="20" rx="2" ry="2"/><path d="M9 22v-4h6v4"/><path d="M8 6h.01"/><path d="M16 6h.01"/><path d="M12 6h.01"/><path d="M12 10h.01"/><path d="M12 14h.01"/><path d="M16 10h.01"/><path d="M16 14h.01"/><path d="M8 10h.01"/><path d="M8 14h.01"/></svg>`,
  curriculum: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>`,
  innovation: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.7 1.3 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg>`,
...
const academicYearsData = {
  '2026': {
    year: '2026',
    kicker: 'NEW GOALS | NEW LEARNING | BRIGHTER FUTURES',
    desc: 'The 2026 academic year at SIET focuses on academic excellence, skill development, research and innovation. Students are encouraged to participate in workshops, seminars, industrial visits and co-curricular activities to gain real-world exposure and build a strong career foundation.',
    btnText: 'Explore 2026 Highlights →',
    btnHref: '#/curriculum',
    stats: [
      { num: '18+', label: 'Courses & Programs', icon: 'courses' },
      { num: '42+', label: 'Faculty Members', icon: 'faculty' },
      { num: '76+', label: 'Laboratories & Facilities', icon: 'labs' },
      { num: '128+', label: 'Industry Collaborations', icon: 'industry' }
    ]
  },
  '2025': {
    year: '2025',
    kicker: 'INNOVATION | INDUSTRY CONNECT | EXCELLENCE',
    desc: 'The 2025 academic session marked milestone advancements in autonomous curriculum enhancements, multidisciplinary laboratory upgrades, and key strategic industry partnerships across emerging domains.',
    btnText: 'Explore 2025 Highlights →',
    btnHref: '#/curriculum',
    stats: [
      { num: '16+', label: 'Courses & Programs', icon: 'courses' },
      { num: '38+', label: 'Faculty Members', icon: 'faculty' },
      { num: '70+', label: 'Laboratories & Facilities', icon: 'labs' },
      { num: '115+', label: 'Industry Collaborations', icon: 'industry' }
    ]
  },
  '2024': {
    year: '2024',
    kicker: 'RESEARCH EXPANSION | SKILL DEVELOPMENT',
    desc: 'During 2024, SIET prioritized student-led patents, global hackathon achievements, and experiential industry-aligned learning pathways for next-generation engineers.',
    btnText: 'Explore 2024 Highlights →',
    btnHref: '#/research',
    stats: [
      { num: '15+', label: 'Courses & Programs', icon: 'courses' },
      { num: '35+', label: 'Faculty Members', icon: 'faculty' },
      { num: '65+', label: 'Laboratories & Facilities', icon: 'labs' },
      { num: '102+', label: 'Industry Collaborations', icon: 'industry' }
    ]
  },
  '2023': {
    year: '2023',
    kicker: 'ACADEMIC RIGOUR | APPLIED ENGINEERING',
    desc: 'The 2023 academic year expanded Centres of Excellence, state-of-the-art computational facilities, and interdisciplinary project modules across all departments.',
    btnText: 'Explore 2023 Highlights →',
    btnHref: '#/centres-of-excellence',
    stats: [
      { num: '14+', label: 'Courses & Programs', icon: 'courses' },
      { num: '32+', label: 'Faculty Members', icon: 'faculty' },
      { num: '60+', label: 'Laboratories & Facilities', icon: 'labs' },
      { num: '90+', label: 'Industry Collaborations', icon: 'industry' }
    ]
  },
  '2022': {
    year: '2022',
    kicker: 'FOUNDATIONS | RESILIENCE | GROWTH',
    desc: 'The 2022 academic calendar strengthened digital learning infrastructure, student mentorship initiatives, and international conference publications.',
    btnText: 'Explore 2022 Highlights →',
    btnHref: '#/academic-calendar',
    stats: [
      { num: '14+', label: 'Courses & Programs', icon: 'courses' },
      { num: '30+', label: 'Faculty Members', icon: 'faculty' },
      { num: '55+', label: 'Laboratories & Facilities', icon: 'labs' },
      { num: '80+', label: 'Industry Collaborations', icon: 'industry' }
    ]
  }
};
...
function academicsOverviewPage() {
  ...
  const data = academicYearsData[currentYear];
  const years = ['2026', '2025', '2024', '2023', '2022'];

  return `<main class="acad-overview-page">
    <section class="siet-vm-hero">
      ...
      <div class="siet-vm-shell siet-vm-hero-content reveal">
        <p class="siet-vm-kicker"><i></i> ACADEMICS</p>
        <h1>Academic <em>Overview</em></h1>
        <p class="siet-vm-intro">Building a strong foundation for a brighter future. At Sri Shakthi College of Engineering and Technology, we provide a dynamic academic environment that blends knowledge, innovation, practical learning and industry exposure to shape confident and capable professionals.</p>
      </div>
    </section>

    <div class="acad-shell">
      <section class="acad-timeline-card reveal">
        <div class="acad-timeline-layout">
          <div class="acad-timeline-nav" role="tablist" aria-label="Academic Years">
            <div class="acad-timeline-track"></div>
            ${years.map(y => `
              <button class="acad-year-btn ${y === currentYear ? 'active' : ''}" type="button" data-year="${y}" role="tab" aria-selected="${y === currentYear}">
                <span>${y}</span>
                <span class="acad-arrow-icon">→</span>
              </button>
            `).join('')}
            <div class="acad-year-hint">
              ${acadIcon('calendar')}
              <span>Click on a year to view the academic overview</span>
            </div>
          </div>

          <div class="acad-year-info">
            <div class="acad-info-kicker">ACADEMIC YEAR</div>
            <h2 class="acad-info-year" id="acad-active-year">${data.year}</h2>
            <div class="acad-info-tags" id="acad-active-kicker">${data.kicker}</div>
            <p class="acad-info-desc" id="acad-active-desc">${data.desc}</p>
            <a href="${data.btnHref}" class="acad-btn-secondary" id="acad-active-btn">${data.btnText}</a>
          </div>

          <div class="acad-glance-card">
            <div class="acad-glance-header">
              <span class="acad-glance-script" id="acad-glance-title">${data.year} at a Glance</span>
            </div>
            <div class="acad-glance-stats" id="acad-glance-stats-grid">
              ${data.stats.map(s => `
                <div class="acad-stat-box">
                  <div class="acad-stat-icon-circle">
                    ${acadIcon(s.icon)}
                  </div>
                  <strong class="acad-stat-value">${s.num}</strong>
                  <span class="acad-stat-label">${s.label}</span>
                </div>
              `).join('')}
            </div>
          </div>
      ...
      <section class="acad-highlights-card reveal">
        <div class="acad-highlights-layout">
          <div class="acad-highlights-heading">
            <span class="acad-highlights-script">Key<br>Highlights</span>
            <span class="acad-highlights-bar"></span>
          </div>

          <div class="acad-highlight-item">
            <div class="acad-highlight-icon">${acadIcon('faculty')}</div>
            <div class="acad-highlight-content">
              <strong>Updated Curriculum</strong>
              <span>Industry-relevant and future-focused.</span>
            </div>
          </div>

          <div class="acad-highlight-item">
            <div class="acad-highlight-icon">${acadIcon('labs')}</div>
            <div class="acad-highlight-content">
              <strong>Modern Laboratories</strong>
              <span>Hands-on learning with advanced facilities.</span>
            </div>
          </div>

          <div class="acad-highlight-item">
            <div class="acad-highlight-icon">${acadIcon('courses')}</div>
            <div class="acad-highlight-content">
              <strong>Expert Faculty</strong>
              <span>Guidance from experienced and skilled mentors.</span>
            </div>
          </div>

          <div class="acad-highlight-item">
            <div class="acad-highlight-icon">${acadIcon('innovation')}</div>
            <div class="acad-highlight-content">
              <strong>Research &amp; Innovation</strong>
              <span>Encouraging new ideas and creative solutions.</span>
            </div>
          </div>

          <div class="acad-highlight-item">
            <div class="acad-highlight-icon">${acadIcon('industry')}</div>
            <div class="acad-highlight-content">
              <strong>Industry Collaboration</strong>
              <span>Bridging the gap between academia and industry.</span>
            </div>
          </div>
        </div>
      </section>

      <section class="acad-impact-card reveal">
        <div class="acad-impact-layout">
          <div class="acad-impact-heading">
            <span class="acad-impact-script">Our Impact</span>
            <span class="acad-impact-bar"></span>
          </div>

          <div class="acad-impact-metric">
            <div class="acad-impact-icon">${acadIcon('faculty')}</div>
            <div class="acad-impact-data">
              <strong>20+ Programs</strong>
              <span>Diverse courses for a brighter future</span>
            </div>
          </div>

          <div class="acad-impact-metric">
            <div class="acad-impact-icon">${acadIcon('courses')}</div>
            <div class="acad-impact-data">
              <strong>200+ Faculty Members</strong>
              <span>Expert mentors and guides</span>
            </div>
          </div>

          <div class="acad-impact-metric">
            <div class="acad-impact-icon">${acadIcon('labs')}</div>
            <div class="acad-impact-data">
              <strong>50+ Laboratories</strong>
              <span>State-of-the-art facilities</span>
            </div>
          </div>

          <div class="acad-impact-metric">
            <div class="acad-impact-icon">${acadIcon('briefcase')}</div>
            <div class="acad-impact-data">
              <strong>100% Placement Support</strong>
              <span>Career ready and future driven</span>
            </div>
          </div>

          <a href="#/placements" class="acad-btn-gold-pill">View All Highlights →</a>
        </div>
      </section>
...
function enhancedHeader() { return `<div class="notice"><div class="notice-track"><span><b>ADMISSIONS 2026–27 NOW OPEN</b><i></i> Applications are invited for undergraduate and postgraduate engineering programmes <i></i> Begin your journey at Sri Shakthi <i></i> TNEA Counselling Code: 2727</span><span aria-hidden="true"><b>ADMISSIONS 2026–27 NOW OPEN</b><i></i> Applications are invited for undergraduate and postgraduate engineering programmes <i></i> Begin your journey at Sri Shakthi <i></i> TNEA Counselling Code: 2727</span></div></div><header class="institution-header-v4 exact-image-header"><div class="institution-header-shell"><a class="siet-header-image" href="#/" aria-label="Sri Shakthi Institute of Engineering and Technology home"><img src="/brand/siet-exact-header.png" alt="Sri Shakthi Institute of Engineering and Technology" width="2048" height="256"></a><nav class="institution-navbar" aria-label="Main navigation"><button class="institution-mobile-toggle" aria-label="Open navigation menu" type="button">${icon('menu')}</button><a class="institution-mobile-logo" href="#/" aria-label="Sri Shakthi Home"><img src="/brand/siet-logo.png" alt="Sri Shakthi" class="mobile-logo-img"><span class="mobile-logo-text"><b>SRI SHAKTHI</b><small>Autonomous Institution</small></span></a><a class="institution-home" href="#/" aria-label="Home">${icon('home')}</a><div class="institution-menu">${pageGroups.map((g, i) => `${i === 5 ? '<a class="institution-nav-link" href="#/placements">Placements</a>' : ''}<div class="institution-nav-group"><button type="button">${g.label}${icon('down')}</button><div>${g.items.map(([s, n]) => `<a href="#/${s}">${n}</a>`).join('')}</div></div>`).join('')}<a class="institution-nav-link" href="#/careers">Careers</a></div><a class="institution-nav-apply" href="#/apply">Apply Now ${icon('arrow')}</a></nav></div></header><div class="mobile-nav-backdrop"></div><aside class="mobile-nav" aria-label="Mobile Navigation"><div class="mobile-nav-header"><a href="#/" class="mobile-nav-brand"><img src="/brand/siet-logo.png" alt="Sri Shakthi"><div><strong>SRI SHAKTHI</strong><small>Autonomous Institution</small></div></a><button class="mobile-nav-close" aria-label="Close menu">${icon('close')}</button></div><div class="mobile-nav-body"><a href="#/" class="mobile-nav-link mobile-nav-home">${icon('home')} Home</a><div class="mobile-nav-accordion">${pageGroups.map((g, i) => `${i === 5 ? '<a class="mobile-nav-link" href="#/placements">Placements</a>' : ''}<div class="mobile-nav-group"><button type="button" class="mobile-nav-group-toggle" aria-expanded="false"><span>${g.label}</span>${icon('down')}</button><div class="mobile-nav-subitems">${g.items.map(([s, n]) => `<a href="#/${s}" class="mobile-nav-sublink">${n}</a>`).join('')}</div></div>`).join('')}<a class="mobile-nav-link" href="#/careers">Careers @ SIET</a></div></div><div class="mobile-nav-footer"><a class="mobile-apply-link" href="#/apply">Apply Now ${icon('arrow')}</a></div></aside>` }
function render() { if (!appRoot) return; const r = route(); let content = !r ? homePage() : r === 'vision-mission' || r === 'about' ? visionPage() : r === 'program-outcomes' ? programOutcomesPage() : r === 'core-values' ? coreValuesPage() : r === 'philosophy' ? philosophyPage() : r === 'chairman' ? chairmanPage() : r === 'principal' ? principalPage() : r === 'academics' ? academicsOverviewPage() : r === 'admission-enquiry' || r === 'apply' ? enquiryPage(r === 'apply') : r === 'careers' ? careersPage() : internalPage(r); appRoot.innerHTML = enhancedHeader() + content + footer(); document.title = `${r ? titleCase(r.replaceAll('-', ' ')) : 'Sri Shakthi'} | SIET`; bind(); scrollTo(0, 0) }
...
function bind() {
  ...
  $$('.institution-nav-group>button').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      if (!group) return;
      const wasOpen = group.classList.contains('open');
      $$('.institution-nav-group').forEach(g => g.classList.remove('open'));
      if (!wasOpen) group.classList.add('open');
    });
  });
  document.addEventListener('click', e => {
    if (!e.target.closest('.institution-nav-group')) {
      $$('.institution-nav-group').forEach(g => g.classList.remove('open'));
    }
  });
  $$('.institution-nav-group>div a').forEach(a => {
  ...
  $$('.acad-year-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const year = btn.dataset.year;
      const data = academicYearsData[year];
      if (!data) return;

      $$('.acad-year-btn').forEach(b => {
        const isActive = b === btn;
        b.classList.toggle('active', isActive);
        b.setAttribute('aria-selected', isActive ? 'true' : 'false');
      });

      const yrEl = $('#acad-active-year');
      const kickEl = $('#acad-active-kicker');
      const descEl = $('#acad-active-desc');
      const btnEl = $('#acad-active-btn');
      const glanceTitle = $('#acad-glance-title');
      const glanceGrid = $('#acad-glance-stats-grid');

      if (yrEl) yrEl.textContent = data.year;
      if (kickEl) kickEl.textContent = data.kicker;
      if (descEl) descEl.textContent = data.desc;
      if (btnEl) {
        btnEl.textContent = data.btnText;
        btnEl.setAttribute('href', data.btnHref);
      }
      if (glanceTitle) glanceTitle.textContent = `${data.year} at a Glance`;
      if (glanceGrid) {
        glanceGrid.innerHTML = data.stats.map(s => `
          <div class="acad-stat-box">
            <div class="acad-stat-icon-circle">
              ${acadIcon(s.icon)}
            </div>
