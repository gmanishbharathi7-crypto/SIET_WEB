...
function render(){if(!appRoot)return;const r=route();let content=!r?homePage():r==='vision-mission'||r==='about'?visionPage():r==='program-outcomes'?programOutcomesPage():r==='core-values'?coreValuesPage():r==='philosophy'?philosophyPage():r==='chairman'?chairmanPage():r==='principal'?principalPage():r==='admission-enquiry'||r==='apply'?enquiryPage(r==='apply'):r==='careers'?careersPage():internalPage(r);appRoot.innerHTML=enhancedHeader()+content+footer();document.title=`${r?titleCase(r.replaceAll('-',' ')):'Sri Shakthi'} | SIET`;bind();scrollTo(0,0)}
